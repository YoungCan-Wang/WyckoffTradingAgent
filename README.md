...
`mcp_server.py` 保留兼容入口；20 个公开工具由 `integrations/public_mcp/` 管理。
...
- **MCP Server** — 20 个公开工具，契约、权限与业务执行分层；[安装与安全边界](docs/PUBLIC_MCP.md)
...
加入 **「威科夫策略交流学习」知识星球** 后，可把登录账号绑定为星球会员，使用云端持仓、最近 30 个复盘交易日跟踪、策略归因、隔离研究计算、手机遥控、每日漏斗产物、影子纸面账详账和专属交流。模型和数据源 Key 仍由用户自行管理；普通用户也可以自配 Key 使用读盘、单股、多股、手动持仓诊断、导出、本地历史和纸面账橱窗。完整边界见 [docs/PLANET_MEMBERSHIP.md](docs/PLANET_MEMBERSHIP.md)。
...
分页失败保留已取消息；多股票主体不明和复牌预告单列，不当成买入标的或当前可交易状态。
