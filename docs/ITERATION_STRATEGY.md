...
2026-08-13 对该门槛另做跨市场阶段因子消融：997只分层样本、约147万根日K、28,578个可执行事件。
被门槛拦截组的T+1/T+5净收益没有弱于放行组；取消门槛相对保留门槛的按日等权T+5差为+0.058个百分点，
95%置信区间跨0。显著差异只存在于波动：取消门槛后5日MFE增加约0.58个百分点、MAE恶化约0.38个百分点。
因此20%不是有效的方向分界，生产实现已移除硬否决，改为20%–30%高波动、30%以上极高波动风险标签，
并仅在趋势延续候选排序中施加最高0.08的轻度惩罚；L4与跨日确认保持不变，OMS把带该风险标签的
`ATTACK`确定性降级为`PROBE`，不禁止试仓。
