...
| 快照 PIT 股票池 | 五窗口 PIT 重跑后的胜率与平均收益 | 退市已补齐并重跑：五窗口平均收益全负、胜率 24%–39%，「无 alpha」结论未被推翻；ST 属按设计排除，剩 `name_map` PIT 名称未做 |
...
| P1 | `name_map` 记录 PIT 名称 | 见「更正：ST 不是偏差来源」；退市已补齐、ST 属按设计排除，剩下的缺口是「当年正常、后来变 ST」的股票被今日名称误滤。须按 as-of 还原名称 |
