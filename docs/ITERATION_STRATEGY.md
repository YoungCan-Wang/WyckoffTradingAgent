...
| 快照 PIT 股票池 | 应有池 vs 旧快照池的缺口构成（退市 / ST） | 取数侧已实现：缺口 4.1%–10.8%，越早窗口越大；现有快照仍旧口径，需重跑 |
...
**注意这不会自动修正已有结论。** 现有 `snapshot_data/` 仍是旧口径生成的；需要重跑快照抓取才会带上补齐后
的股票池。重跑前的任何回测结果仍带此偏差，方向为乐观。
...
| P1 | 扩大 loss guard 覆盖 | 实盘数据显示过滤器有 +7.5pct 增益；与上一条解绑——砍排序分会削弱 `_exit_penalty`，两者方向相反 |
...
| P1 | 用 PIT 股票池重跑快照，复核受影响结论 | 取数侧已实现（见「快照股票池带幸存者偏差 / 已实现」）；**现有 `snapshot_data/` 仍是旧口径**，需重跑才生效。重跑后须复核早期窗口（bull_2020 缺口 10.8%）的结论是否变化 |
