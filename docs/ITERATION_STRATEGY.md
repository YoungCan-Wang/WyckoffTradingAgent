...
| Step3 财务字段 | 财务档内 `ai_recommended` 与未采纳者的超额差 | `blocked`：`ai_recommended` 连续两月为 0、送审量塌缩 40 倍，验收缺一侧样本；不得声称正向收益 |
...
五只**全部通过 L1**，所以与财务数据接线无关，也与市值、流动性门槛无关。四只倒在 L2 的
`trend_cont_max_drawdown_pct=20.0`（60 日窗口）：实际回撤 26%–39%，且均线均非多头排列，两项独立条件同时
不满足。更关键的是**五只的买点触发全部为空**——即使放宽 L2，它们当天也不会成为候选。
...
## Step3 财务字段的验收协议（2026-08-09 登记，`blocked`）
...
| 月份 | observation | `selected_for_ai` | `ai_recommended` |
|---|---:|---:|---:|
| 2026-05 | 205 | 205 | 43 |
| 2026-06 | 1,194 | 138 | 21 |
| 2026-07 | 2,897 | 31 | **0** |
| 2026-08 | 464 | 5 | **0** |

**`ai_recommended` 已连续两个月为 0，送审量从 5 月的 205 塌缩到 8 月的 5（近 27 个交易日日均 1.33）。**
按此速率，150 个送审样本需约 112 个交易日，而不是原以为的 30 个。采纳数为 0 意味着「档内比较
`ai_recommended` 与未采纳者」这个设计**当前根本无法执行**——一侧样本恒空。

采纳为 0 与市场闸门方向一致：07-01 起 28 天里收盘态属禁买/限买档的有 19 天（CRASH 7、PANIC_REPAIR 4、
BEAR_REBOUND 4、RISK_OFF 3、CAUTION 1），NEUTRAL 仅 8 天。但这只是方向一致，不构成完整解释——送审量
本身也塌了 40 倍，而送审发生在闸门之前。这与「漏斗体检」记录的 Step3 起跳板恒空、以及送审池收窄到
默认最多 5 只有关，需要单独核实，不在本节范围。

### 预注册口径（在样本流恢复之后才可执行）

1. **主检验是「财务字段能否解释 LLM 判定的分歧」，不是修复前后对比。** 按当日可得的财务快照分档
   （ROE 正/负、净利润同比正/负、资产负债率高/低），比较**同一档内** `ai_recommended` 与未采纳者的
   后续超额。若财务字段在起作用，采纳决策应当在财务较差的档位里表现出更强甄别力。
2. **基准用日期配对全市场等权**，同入场同退出日，与本文档其它截面结论同口径。
3. **样本门槛**：至少 150 个可评价送审样本、其中 `ai_recommended` 与未采纳各不少于 40 个，
   且覆盖至少两种 `benchmark_regime`。**按当前速率不可达**，须先解决样本流问题。
4. **同时报告负控制**：财务档位随机打乱后重跑，档内差值应塌缩到噪声区间。
5. **跨期前后对比只作为敏感性结果**，须显式标注 regime 差异，不得作为晋级依据。
...
**当前状态 `blocked`，不是 `exploring`。** 阻塞原因不是等时间，而是 `ai_recommended` 恒为 0、送审量塌缩
40 倍，验收设计缺一侧样本。因此：

- **不得声称该修复带来正向收益**，也不得声称「等两个月就能验收」——按现速率是 112 个交易日，且采纳侧仍可能为 0。
- **优先级高于本验收项的是先查清送审量塌缩与 `ai_recommended` 恒零的原因**（已登记为 P0）。那是一个
  产出分布异常，符合「漏斗体检」总结的三种故障形态之一：零产出、恒定产出、产出与输入无交集。
...
因子无可交易 alpha 的结论。形态信号回放仍需确保 `workflows/backtest_snapshot_fetch.py` 的股票池包含
历史退市与当时 ST 标的。
...
| P0 | 查清 Step3 送审量塌缩与 `ai_recommended` 恒零 | 实测 `selected_for_ai` 月度 205→138→31→5，`ai_recommended` 07/08 两月为 0；属「零产出」故障形态，且阻塞一切依赖送审样本的验收 |
...
| P2 | 验收 Step3 财务字段 | 见「Step3 财务字段的验收协议」；被上一条阻塞，样本流恢复前无法执行 |
