...
| Layer 2 两轨制 | 变体 J（仅开 `enable_two_track_mode`）对照 A 的六窗口现金收益、边际成交与 L2 放行数 | 否决（#476）：三胜三负、均值 +0.52pp 全部来自 recent_6m；J 新增成交均收 −2.07% 差于其踢掉的 −1.17%；1125 个交易日里 1078 天 L2 放行更少，「提升召回」前提为反。两轨制函数、`FunnelConfig` 参数与变体 J/L/N 已删除 |
...
| 新闻 / 舆情 | `rag_veto`（AkShare 个股新闻）+ CLI `browser_research`（本机 CDP）+ 单股分析页新闻打点 + `scan_corporate_events`（重组/停牌观察） | 图表叠加与公司大事扫描只做读盘解释；要找 alpha 仍须先做事件日前后的异常收益，而不是再加一层形态阈值 |
