...
| A股实证入场 | P 风控基线上的信号质量门控 | 默认回测 P/Q/R/S/T，固定 Top-N 后拦截且不补位；未验证前不改变生产漏斗 |
...
2. **跨周期有效**：近期、牛市、熊市、震荡市和高波动市都要覆盖，不能用单段行情代替稳健性。
...
新的默认消融固定为 `P/Q/R/S/T`。P 保留上一轮表现最好的仓位风控，Q 禁止 NEUTRAL Spring，R 禁止
LPS，S 禁止 NEUTRAL EVR，T 禁止 CAUTION SOS。四个候选都先固定 P 的 Top-N，再在重放阶段拦截目标
信号并留空，不允许过滤后补位。默认晋级证据覆盖近期六个月、2020 牛段、2022 熊段、2023 震荡和 2024
高波动五个窗口；最近两个月只作新鲜度诊断，不参与晋级判定。

P/Q/R/S/T 的触发器、排序和基础候选完全相同，因此每个窗口只建立一次 P 信号台账，再重放五种门控与
现金组合。共享兼容性只放行重放阶段的信号拦截和仓位权重；广度确认、分数校准、持有期等会改变信号生成
或执行路径的配置仍拒绝共享。这样新增四组门控不会把全市场漏斗扫描放大五倍。
...
标记 `pass`。P/Q/R/S/T 都是回测研究策略，不会因单次胜出自动改写
生产漏斗、Step3 或 OMS。

run #89 的五窗口结果中，P 平均现金收益仍为 −0.88%，在 2023 震荡和 2024 高波动窗口分别亏损 12.71%
与 10.83%，只能保持 `review`。P 的实际成交显示 NEUTRAL Spring、LPS、NEUTRAL EVR 和 CAUTION SOS
分别在 4/5 个窗口表现较差，因此下一轮只做四个单变量不补位门控，先判断选股质量是否改善，避免继续用
缩仓掩盖负期望。策略对比报告保留 P 的实际成交归因，并分别给出 Q/R/S/T 相对 P 的处理暴露和跨窗口结果。
