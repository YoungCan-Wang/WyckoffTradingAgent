...
| A股实证入场 | 弱水温仓位与 NEUTRAL Spring 缩仓 | 默认回测 A/M/P，未验证前不改变生产漏斗；Q 广度确认已退出默认矩阵 |
...
信号仓位；P 在 M 上仅将 NEUTRAL Spring 仓位由 50% 降至 25%，验证更小的左侧试探仓是否改善风险收益。默认对比覆盖近期六个月、2020 牛段、2022
熊段、2023 震荡和 2024 剧烈波动五个窗口，P 必须相对 M 评估。Q 的广度确认在完整五窗口中相对 P 仅 1/5 胜、平均收益差
−3.85pp、最大回撤最多恶化 6.52pp，故退出默认矩阵，只保留手工复现能力。
...
## 连续历史检验：合成价值分暂不推广（2026-07-29，待当前代码复核）

> **当前代码复核状态（2026-07-30）**：以下结果来自修复前的运行，尚未以当前 `main` 复跑。
> Actions 抓取脚本早已规范化带连字符日期，且每次运行都会重新写入 `universe.parquet` 和
> `name_history.parquet`；因此不能断言旧年面板必然漏了交易日或保留了旧 ST 标记。缓存代际 `v2` 的目的
> 是为当前 PIT 构建语义提供一次干净、可复现的基线。复跑前，这些数字只支持「不推广、不调参」，不能作为
> 合成价值分被最终证伪的结论。复跑必须确认 cache miss、年度面板未被跳过，并在相同八格参数下重新比较
> alpha 与可投资覆盖。
...
`v1` 暂定结论：IC 显著 ≠ 可交易。Spearman IC 在秩空间给每只票同等权重，而收益由少数大波动主导；
`IC = 0.05` 这个量级的单调倾斜，在任何长仓实现里都会被个股特异波动淹没。`v2` 冷缓存复跑若仍为全负
alpha，才将价值类因子在这套数据和资金规模下正式冻结；在此之前不新增因子变体，也不接入漏斗或 OMS。
