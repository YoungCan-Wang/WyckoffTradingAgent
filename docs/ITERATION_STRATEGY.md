...
**601138 是本轮唯一值得单独跟进的线索，但不构成召回缺陷。** 它的止损参考价 75.58 锚定 60 日内最高价
（2026-06-03 的 83.98），而该股已从 53.18 回升 28% 至 68.18、重新站上 MA50（66.91）、处于 20 日收盘新高，
仍被判「深度破位」。`_compute_stop_loss` 的跟踪止损只随 60 日高点走、不随修复走，因此深跌后强修复的票会
在整段修复期持续挂着 stop_loss；而 `_exit_penalty` 对 stop_loss 给 −100 分，等于永久否决。这与「实盘反馈
验收」里止损阈值不可标定的结论同源，属于退出口径问题，须按那一节的纪律单独立项验证，不在本次改动范围。
...
| P1 | 跟踪止损在深跌后强修复段的口径 | 601138 已站上 MA50、处 20 日新高，仍按 60 日前高点判「深度破位」并被 `_exit_penalty` 永久否决；须按「实盘反馈验收」的纪律独立立项，不得直接调阈值 |
