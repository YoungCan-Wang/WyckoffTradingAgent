...
−3.85pp、最大回撤最多恶化 6.52pp，故实验实现已删除，不再占用维护和复跑资源。
由于 A/M/P 不改变候选集合、排序和退出规则，Backtest Grid 现在每个窗口只建立一次信号台账，再按三组
权重分别重放交易与现金组合；运行前会移除权重字段比较其余策略配置，任何候选或触发规则差异都会直接
拒绝共享。这样把五窗口默认消融从 15 次完整漏斗扫描降为 5 次，研究口径不变。
