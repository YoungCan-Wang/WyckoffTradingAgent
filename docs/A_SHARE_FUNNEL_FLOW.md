...
漏斗 Step2/3 成功后，`workflows/shadow_ledger_job.py` 另跑一套纸面账本：买许可与 Step4 规则准入相同，但只写 `shadow_*`。盘后定计划、次日官方开盘价成交，遵守 T+1 / 整手 / 涨跌停 / 费用；失败只记日志，不阻断漏斗。独立飞书卡标题为「影子账本 / paper」，不是实盘，也不是 `ic_shadow`。持仓区只列仍开着的仓，逐只展示盯市净收益（市值 − 含买费成本）及百分比；已卖光的名字不出现。账户汇总的现金 / 净值 / 市值 / 累计不变。Web `/shadow` 只读同一套 `USER_SHADOW:*` 表：非会员看橱窗，有效星球会员看日流水与持仓净收益；服务端裁剪 payload。
