...
公司大事 / 停牌扫描（`corporate_event_scan.yml`）是旁路观察：读已公告与媒体电报，不进本漏斗、不改 OMS。个股新闻打点只在已经打开某只股票时生效，该入口仅检索有限媒体来源，不能据零命中断言无事件；来源状态与精确48小时窗口见 [扫描契约](CORPORATE_EVENT_SCAN.md)。
...
漏斗 Step2/3 成功后，`workflows/shadow_ledger_job.py` 另跑一套纸面账本：买许可与 Step4 规则准入相同，但只写 `shadow_*`。盘后定计划、次日官方开盘价成交，遵守 T+1 / 整手 / 涨跌停 / 费用；失败只记日志，不阻断漏斗。独立飞书卡标题为「影子账本 / paper」，不是实盘，也不是 `ic_shadow`。持仓区只列仍开着的仓，逐只展示盯市净收益（市值 − 含买费成本）及百分比；已卖光的名字不出现。账户汇总的现金 / 净值 / 市值 / 累计不变。Web `/shadow` 只读同一套 `USER_SHADOW:*` 表：未登录只进登录页；登录后非会员看橱窗，有效星球会员看日流水与持仓净收益；服务端裁剪 payload。
...
I（确认分校准）单独重跑全市场漏斗，报表按 `backtest-strategy-<period>-<variant>/` 目录名归组后与同窗口的 A
对照，不合命名的产物目录会在报表里列为「已忽略」而不是静默缺席。报表另按 signal_date+code 配对给出每组相对参照的
边际成交（共同 / 仅参照 / 仅本组的笔数、均收、胜率），回答「换进来的票是否比换掉的好」。Layer 2 两轨制 J（#476）曾以
同样方式独立分片对照 A：六窗口三胜三负、J 新增成交均收比其踢掉的更差、L2 放行在全部窗口更少，已否决并删除。
