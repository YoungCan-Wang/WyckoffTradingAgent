...
- `contracts.py`: 20 public tool names (including `scan_corporate_events`), input defaults, JSON Schemas and conservative annotations.
...
- `agents/public_mcp_backend.py`: lazy principal/context initialization and the shared production `ToolSurface`.
- `agents/public_mcp_handlers.py`: the existing funnel parameter adaptation, not a replacement trading algorithm.
