...
| **運用プレイブック** | [OPERATOR_PLAYBOOK.md](OPERATOR_PLAYBOOK.md) |
...
| 主線ファネルスクリーニング | A株全市場と香港株 / 米国株の独立 universe を、動的テーマ、8チャネル強度、候補レーン、タイミング確認で走査 |
...
| 翌日寄り付き執行 | クロスデイ確認後、OMS が唯一の許容買い区間を生成し、翌日の寄り付きが区間内の場合だけ執行 |
...
| `record_trade_fill` | 約定の反映：平均取得単価の再計算、手数料控除、実現損益の算出 |
| `get_market_overview` | 市場全体の温度感 |
| `screen_stocks` | 主線ファネルによる全市場スクリーニング（⚡バックグラウンド） |
| `generate_ai_report` | 3陣営 AI 詳細レポート（⚡バックグラウンド） |
| `generate_strategy_decision` | 保有銘柄の去就 + 新規エントリー判断（⚡バックグラウンド） |
| `query_history` | 過去の推奨 / シグナルプール / 帰属・コンテキストアーカイブの照会 |
...
| L2 | 8チャネル強度 | 主要上昇 / 点火 / 潜伏 / アキュムレーション / 閑散出来高 / 支持 / トレンド継続 / ブレイク加速 |
| Mainline | テーマエンジン | 概念熱度、テーマレーダー、財務品質、タイミング確認で主線候補を検出 |
| L3 | セクター・概念共鳴 | 弱いセクターを除外し、強い個別株と確認済みテーマは固定 Top-N 制限を迂回可能 |
| L4 | ミクロ狙撃 | Spring / LPS / SOS / EVR / Compression / Trend Pullback |
| L5 | AI + OMS 審判 | LLM レビュー、クロスデイ確認、OMS リスクゲートを通過してから行動 |


**運用:** 日次ファネル=候補と環境、**confirmed** かつ翌日寄り付きが OMS の唯一許容区間内にある候補だけを買う。NEUTRAL が主戦場、**RISK_ON は新規禁止**。詳細は [OPERATOR_PLAYBOOK.md](OPERATOR_PLAYBOOK.md)。

## 日次自動化

GitHub Actions と Codex Automation による日次自動化：

| タスク | 時刻（北京時間） | 説明 |
|--------|-----------------|------|
| ファネル + AI レポート + プライベート判断 | 日-木 17:17 | 完全自動、結果を Feishu / Telegram にプッシュ |
| 確認 + 翌日寄り付き | `confirmed` 後の日次フロー | LLM 再評価 + OMS ゲート。独立した日中タスクなし |
| プレマーケットリスク管理 | 月-金 08:20 | Codex Automation が GitHub workflow を起動；A50 + VIX アラート |
| 強い銘柄の復習 | 月-金 19:25 | 当日終値騰落率 > 7%、前営業日終値騰落率 < 3% の銘柄を復習 |
| レコメンド追跡リプライシング | 月-金 23:00 | 終値を同期 |
| バックテストグリッド | 毎月1日・15日 04:00 | 8重点パラメータ → 集約レポート |
| DB メンテナンス | 火-土 06:20 | 相場、注文、シグナル、市場シグナルなどのローリングデータをクリーンアップ |
