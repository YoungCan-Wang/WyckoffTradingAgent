...
漏斗负责**发现机会并生产信号样本**，feedback 负责**盘后验收信号表现并更新策略状态**，动态策略在**下一轮漏斗**读取这些状态。外部观察名单只进入同一套观察闭环，不作为正式候选来源。默认不是强同步链路，而是错峰运行的反馈闭环。Health 行额外带 IC / Sharpe 与 `decay_lifecycle`；默认只观测，`SIGNAL_DECAY_APPLY=1` 才改 registry。
