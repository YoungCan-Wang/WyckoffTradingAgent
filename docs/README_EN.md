...
The Electron desktop workspace brings Agent chat, approvals, portfolio, schedules, tracking, attribution, reports, and K-line charts into one local application. Public downloads belong on [GitHub Releases](https://github.com/YoungCan-Wang/WyckoffTradingAgent/releases); Settings → General → Software updates shows the installed version and links a newer `desktop-v*` release when available. CI builds and verifies Windows x64 and macOS Intel/Apple Silicon candidates with the bundled Python runtime without uploading the large files on push or PR; an explicitly requested manual candidate is kept for at most one day. Public packages deliberately use the zero-cost unsigned Windows / ad-hoc-signed macOS path and disclose the resulting operating-system warnings. See [DESKTOP_RELEASE.md](DESKTOP_RELEASE.md) for automated publishing, storage cleanup, and release requirements.
