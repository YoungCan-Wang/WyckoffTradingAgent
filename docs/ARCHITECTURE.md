...
| **策略消融** (`backtest_grid.yml: strategy_compare`) | 随回测网格触发 | 复用同一快照并行运行 A/M/P；M 相对 A 验证弱水温缩仓，P 相对 M 验证将 NEUTRAL Spring 仓位由 50% 降至 25%，并覆盖五个时间窗口。N（过滤后重排）与 O（拦截后不补位）已退出默认矩阵 |
