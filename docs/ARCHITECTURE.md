...
| `/fib` | 斐波那契画线 | 公开页，不经 AuthGuard、不查星球会员。搜索复用股票搜索框；底图是 TradingView Advanced Chart（`SSE:` / `SZSE:`）。点击「生成画线」后，浏览器直接读东方财富前复权日 K，用最近 120 根的最高/最低从低点向上标 0 / 23.6% / 38.2%（赚钱空间下沿）/ 50% / 61.8% / 78.6%–100%（供给区）/ 161.8% 扩展。免费组件没有官方画线接口，水平线是叠在底图上的，缩放后可能错位。构建时复制 `fib.html`，避免 Pretty URLs 把 `/fib` 折回 `/`。 |
