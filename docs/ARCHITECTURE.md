...
| **策略消融** (`backtest_grid.yml: strategy_compare`) | 随回测网格触发 | 复用同一快照并行运行 A/M/P；M 相对 A 验证弱水温缩仓，P 相对 M 验证将 NEUTRAL Spring 仓位由 50% 降至 25%，并覆盖五个时间窗口。Q 的广度确认因五窗口失效而退出默认矩阵，仅保留手工复现；N（过滤后重排）与 O（拦截后不补位）也已退出 |
...
| **截面因子组合回测** (`factor_backtest.yml`) | 手动触发 | 与形态信号回测互不相干的第二条路：自建 PIT 面板（按交易日横切，含退市与 ST），按合成价值分每 K 日重排全市场并持有前 N 只，扫「持仓数 × 再平衡间隔」网格，输出净值、同频等权基准与真实摩擦拆解；PIT 输入语义变更时必须升级面板缓存代际并冷重建 |
...
形态信号回测回答的是「某只票出现形态后单独买卖一笔」，截面组合回测回答的是「每隔 K 个交易日把全市场
重排一次，持有分数最高的 N 只」。持仓是一个整体、换手由排名变动决定，与逐笔台账不可互相套用，因此是
一条独立路径，不复用 `core/backtest_run.py`。

| 模块 | 职责 |
|------|------|
| `integrations/factor_panel.py` | 按 `trade_date` 横切拉 `daily` / `adj_factor` / `daily_basic`，按年落盘 parquet；`namechange` 整表分页还原 PIT 证券简称 |
| `core/factor_scores.py` | 四项价值因子（BP / 盈利收益率 / 销售市值比 / 股息率）按日秩标准化后合成；涨跌停与停牌的可买可卖判定；后复权价按各股首个观测日归一 |
| `core/factor_portfolio.py` | 组合模拟：缓冲式再平衡、一手制与最低佣金、停牌与退市处理、同频等权基准 |
| `workflows/factor_backtest.py` | 参数网格编排与产物（净值、分年度、成交、摩擦拆解） |

关键口径，都是实测踩出来的：

- **股票池按交易日横切构建**，不预先取名单，当天在交易的股票必然在面板里。`snapshot_data/` 那套按
  「拉取当日存续且非 ST」取池，每个历史窗口漏掉约 7.5% 的可交易标的（见 ITERATION_STRATEGY）。
- **基准必须与组合同频再平衡**（`holding_period_benchmark`）。用每日等权线算 alpha 会凭空多出近
  9pct——那是再平衡频率差异，不是选股能力。每日等权线仍作为常规市场参照保留在 `nav.csv` 里。
- **买不起一手要继续沿排名补位**。固定 `equity/top_n` 预算且不补位时，一手价高于每格预算的标的会被
  静默跳过，等于按股价做了一次隐性筛选，实测能造出 8pct 的假 alpha。
- **停牌按截至当日的最后成交价估值**，用该股在整个面板里的最后价格是未来函数。
- **PIT 数据语义变更必须冷重建面板**。工作流以 `PANEL_CACHE_VERSION` 隔离缓存代际；升级该值时不能
  回退到旧代际缓存，因为按年落盘的面板会跳过已存在文件。本地 CLI 不读取 Actions 缓存，需显式换用新的
  `--output-dir`（推荐）或先清理旧目录。
- 回归检验：把分数换成随机数，零摩擦下净值必须贴住基准。该断言固定在
  `tests/core/test_factor_portfolio.py::TestNullHypothesis`。
