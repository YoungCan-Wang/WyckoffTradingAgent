...
Web `/shadow` 只读 `USER_SHADOW:*` 纸面账：`GET /api/shadow-ledger` 用服务端角色查 `shadow_*`，按
`planet_members` 裁剪 payload。未登录或非会员只返回橱窗字段（净值曲线、区间净收益、最大回撤、
粗胜率、开仓只数、板块标签），不含代码/名称/价格/数量/止损；有效会员才返回日净值、事件流水
和仍开仓净收益（公式与飞书卡一致：`shares*last_mark - shares*avg_cost`）。不写实盘表，也不读雷达库。
