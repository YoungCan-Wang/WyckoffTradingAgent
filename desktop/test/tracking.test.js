...
test('涨跌按该行事件价重算', () => {
  const out = dedupeByCode([
    row({ code: 'A', recommend_date: '20260818', recommend_price: 80, current_price: 88, pnl_pct: 0 })
  ])
  assert.equal(out[0].recommend_price, 80)
  assert.equal(out[0].pnl_pct, 10)
})
...
test('跨日倒序输入也保留各自事件价', () => {
  ...
  const later = out.find((r) => r.recommend_date === '20260818')
  const earlier = out.find((r) => r.recommend_date === '20260801')
  assert.equal(later.recommend_price, 80)
  assert.equal(earlier.recommend_price, 50)
  assert.ok(Math.abs(later.pnl_pct - 12.5) < 1e-9)
  assert.ok(Math.abs(earlier.pnl_pct - 80) < 1e-9)
})

test('现价缺失时保留原涨跌而不是算出 NaN', () => {
  const out = dedupeByCode([
    row({ code: 'A', recommend_price: 80, current_price: null, pnl_pct: 12.5 })
  ])
  assert.equal(out[0].pnl_pct, 12.5)
})

test('基准价为 0 不产生 Infinity', () => {
  const out = dedupeByCode([
    row({ code: 'A', recommend_price: 0, current_price: 90, pnl_pct: 5 })
  ])
  assert.equal(out[0].pnl_pct, 5)
  assert.ok(Number.isFinite(out[0].pnl_pct))
})
