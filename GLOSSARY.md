...
| **斐波那契画线** | 公开 Web 页 `/fib`，侧栏在「投研终端」，不要求登录或星球会员。底图是 TradingView。价位按最近 120 根前复权日 K 的最低到最高向上量（与 `channel_geometry` 的黄金分割房间同一方向）：38.2% 是赚钱空间下沿，78.6%–100% 是供给区，161.8% 是扩展目标。只是读盘参考，不进漏斗、不产生买卖指令。 |
