...
| **公司大事 / 停复牌扫描** | `scan_corporate_events`：东财关键词与财联社最新批次的最近48小时观察，不是全量公告核对。`source_status` 区分 ok/partial/unavailable；分页失败保留已取数据，coverage_status 与请求健康分离；否认、终止、复牌预告及主体歧义独立标识；effective_date 不是当前可交易状态，缺失发布时间不算近期。只写观察产物和通知，不改漏斗或 OMS。见 [扫描契约](docs/CORPORATE_EVENT_SCAN.md)。 |
