...
| **A股实证消融 P/Q/R/S/T** | 默认 confirmed-only 实验：P 为仓位风控基线；Q/R/S/T 分别禁止 NEUTRAL Spring、LPS、NEUTRAL EVR、CAUTION SOS。各组固定 P 的 Top-N 后在重放阶段拦截且不补位，共享一次信号台账；五个晋级窗口外附最近两个月诊断。只用于回测，不自动晋级生产 |
