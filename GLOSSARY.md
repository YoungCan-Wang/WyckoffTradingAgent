...
| **A股实证消融 A/M/P** | 默认 confirmed-only 实验：A 基线，M=弱水温信号缩仓，P=M + 将 NEUTRAL Spring 仓位由 50% 再降至 25%。只用于回测，不自动晋级生产。Q 的广度确认在五窗口相对 P 仅 1/5 胜、平均收益差 −3.85pp，已退出默认矩阵，仅保留手工复现；N（过滤后重排）与 O（拦截后不补位）也已证伪 |
