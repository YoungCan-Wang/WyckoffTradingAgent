...
| **推荐池累计涨幅** | 跟踪表 `initial_price` 可能沿用同票首次推荐价，只能解释首次入池以来走势，不能归因到每次重复推荐或次日买入 |
...
| **主线×威科夫交叉** | 两道独立漏斗的交集筛选：Radar 策划主题漏斗（`radar_theme_snapshots` rank≤5 + `radar_symbol_snapshots`）只回答「是否在当日扩散中的策划主题里」；威科夫结构漏斗只回答「个股量价/形态是否偏强」。默认展示 PRIMARY 口径。它是观察筛，不生成 `next_buy`、开盘带、股数或 Step4 工单；市场闸门拦截仍落库（只记不执行）。不是本仓 `theme_radar_snapshot`，也不把东财热榜写入 Radar 主线成立 |
