...
| **L5** | 退出信号 | 持有期间监控是否出现派发信号（SOW / UTAD），决定是否提前退出。持仓诊断只喂入建仓日之后的价格路径；`buy_dt` 缺失或切不出建仓后 K 线时 fail-closed，不发结构退出，建仓前的高点与暴跌不参与破位判定。新多头 `add` 必须写入合法建仓日；`update` 不得在空账本新建无日期持仓 | — |
