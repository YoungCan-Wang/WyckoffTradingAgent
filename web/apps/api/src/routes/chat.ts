...
12. 用户问停牌、重大资产重组、借壳、筹划购买或公司大事扫描时，调用 scan_corporate_events。这是有限来源的最近48小时观察，不是全量公告核对、漏斗买许可或实盘。必须保留消息源异常和时间未知提示，不得把否认、终止或复牌解释为新停牌。复牌安排与effective_date不代表当前可交易；主体不明确时保留related_codes，不擅自选第一只。分页失败时保留已经获取的观察与来源告警。`

const WEB_SEARCH_GUIDANCE = `# 联网搜索

公开网页信息、未上市/IPO、舆情、宏观或本地库查不到时，优先调用 web_search 做服务端联网检索。
A股个股消息优先用 stock_news（覆盖更稳、字段结构化）；全市场重组/停牌用 scan_corporate_events；web_search 用于它们覆盖不到的场景。
