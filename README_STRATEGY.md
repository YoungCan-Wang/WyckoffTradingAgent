...
Step3 默认最多复核 5 只，跨日 `VALIDATED` 候选优先占用最多 3 席，剩余席位才按漏斗顺序从当日 `selected_for_ai` 填充；这既保留当日唯一预选血缘，也不会让已验证信号被上下文 cap 裁掉。报告正文展示执行摘要、实际送审清单，以及 A/B/C≥2 且当日写入 `recommendation_tracking` 的精简形态观察（默认最多 20 只）；完整 L4、主线池和证据仍保留在结构化运行数据中。
...
不会再用建仓前的 60 日高点反杀新仓。新仓保护期内，只有当轮模型给出的止损同时高于成本和现价时，
