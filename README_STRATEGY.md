...
对于未能通过 L2 八通道的个股，系统提供失败缺口数值诊断：计算个股距各启用通道条件的最小相对差距（如价格乖离、量能、RPS等），并按差距绝对值大小进行全局排序。诊断覆盖主升、潜伏、吸筹、地量、暗中护盘、趋势延续、加速突破和点火破局（SOS）全部八个通道；强势股复盘会复用一次构建的全市场 RPS 与基准上下文，避免逐股重复计算。在报表中直接输出“最接近通道(缺口X%): 原因”，以辅助直观分析个股结构演变距离。
...
并行运行五组策略：P 是已验证减亏但尚未晋级的仓位风控基线；Q/R/S/T 分别禁止 NEUTRAL Spring、LPS、
NEUTRAL EVR、CAUTION SOS。四个候选固定 P 的 Top-N 后拦截且不补位，从而隔离单一信号门控的真实贡献。
消融任务关闭动态归因调权，避免外部策略权重掩盖规则本身贡献；全部实验默认不改变生产策略。

消融汇总比较每组与 P 组的 `(signal_date, code, entry_weight_multiplier, exit_date)` 实际成交差异。规则必须
真实改变至少两个周期的交易，并在五个晋级窗口满足现金收益、回撤和 walk-forward 门槛；最近两个月只作
新鲜度诊断。A-E 经典形态消融仍保留手动复验能力，但已退出默认算力矩阵。
...
默认 P/Q/R/S/T 消融也共用一次信号台账：先固定 P 的 Top-N，Q/R/S/T 分别在重放阶段拦截一种目标信号
且不补位，随后重放退出和现金组合。运行时会校验策略差异；广度确认、分数校准、触发器或持有期等会改变
信号生成路径的变体仍拒绝复用，必须完整重跑。
...
当日买入的股份当日不可卖出。持仓按代码聚合、没有分笔明细，因此买入日期等于当前交易日时整个仓位视为冻结：EXIT / TRIM 一律拒单并标注 `T+1限制`，连"跌破止损强制离场"的兜底也只能保持 HOLD 并顺延到下一交易日。持仓 payload 会显式告知模型该票当日不可卖，避免它反复提议无法执行的离场。
