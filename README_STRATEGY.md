...
Step3 默认最多复核 5 只，跨日 `VALIDATED` 候选优先占用最多 3 席，剩余席位才按漏斗顺序从当日 `selected_for_ai` 填充；这既保留当日唯一预选血缘，也不会让已验证信号被上下文 cap 裁掉。报告正文只展示执行摘要、实际送审清单和少量研究结论；完整 L4、主线池和证据保留在结构化运行数据中，不再把数十只观察样本铺进用户报告。
